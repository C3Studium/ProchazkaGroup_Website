 ...
 */
/**
 * Kdo vidí a mění „Zařazení".
 *
 * Členům se ta záložka nezobrazí vůbec — pořadí poradců na webu ani typ záznamu
 * nejsou věci, které redakce upravuje mimochodem. Majitel je vidí a smí je
 * měnit: rozhodnout, jestli je záznam poradce nebo Benefit program, a v jakém
 * pořadí se poradci ukazují, je vlastnická úvaha.
 *
 * Počítadla a původní ID jsou i pro majitele jen ke čtení. Počítadla udržuje
 * web (viz níž), takže ručně zapsané číslo se stejně přepíše, a původní ID je
 * stopa na starou databázi, podle které se pozná, odkud řádek přišel — přepsat
 * ji znamená tu stopu ztratit.
 */
...
export default defineType({
    name: 'consultant',
    title: 'Poradce',
    // Carried ON the type, not only as a named export, so library code can ask
    // "what does a document of this type call itself?" without importing a file
    // that belongs to this site. `defineType` spreads the config onto the frozen
    // type, so this survives registration. See src/cms/site/types.js.
    displayName: consultantFullName,
    icon: 'user',
    groups: [
    ...
    fields: [
        ...
        defineField({
            ...
            description: 'Adresa stránky poradce: /recenze/<url>. Změna rozbije existující odkazy.',
            // Only the administrator. This is the address of a page that is
            // linked from the site, indexed, and — the reason it matters most —
            // printed as a QR code on the consultant's business card. A card
            // cannot be edited once it is in somebody's wallet, so a rename an
            // editor makes to tidy a name breaks a link nobody can fix. The
            // server refuses the write as well (server/adapter.js).
            adminOnly: true,

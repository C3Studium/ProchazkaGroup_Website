...
// from `@/cms/schemas` so the core's registry is cleared first — see
 ...
 *         `@/cms/server/httpDataPort`, never from `@/cms/server`: the barrel
 ...
 * behind an auth gate — there is nothing to server-render, and the public site's
 * `_app` wraps every page in a WebGL canvas and a smooth-scroll controller that
 * the admin has to switch off once it is mounted.
 */
const Studio = dynamic(() => import("@/cms/studio"), {
  ssr: false,
  loading: () => <Booting />,
})
...
export default function StudioPage() {
  // Created once per mount. A port identity that changed on every render would
  // invalidate every data hook in the admin on every keystroke.
  const port = useMemo(() => createLazyPort(loadPort), [])

  return (
    <>
      <Head>
        <title>Studio — Procházka Group</title>
        {/* An admin behind a login has no business in an index. */}
        <meta name="robots" content="noindex, nofollow" />
      </Head>
      <Studio core={core} port={port} config={{ title: "Studio" }} />
    </>
  )
}
